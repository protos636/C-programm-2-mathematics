...
Suite *suite_tan(void) {
  Suite *s = suite_create(
      "suite_tan");  // создает набор , который будет содержать test cases.
  TCase *tc = tcase_create("tan_tc");  // создать test case.

  tcase_add_test(tc, test_tan_positive);  //Добавить тест. ф-цию в test case.
  tcase_add_test(tc, test_tan_negative);  // tcase_add_test(tc,tf) tf test
  ...
  suite_add_tcase(s, tc);  // Add a test case to a suite.s suite to add test
                           // case to.tc test case to add to suite
  return s;
}